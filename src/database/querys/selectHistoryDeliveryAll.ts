export const numero_entregas = `
SELECT 
    COUNT(e.id) AS numero_entregas,
    DATE(e.created_at) AS data_entrega,
    SUM(e.valor_liquido) AS valor_total
FROM entregas AS e
INNER JOIN motoboys m ON m.id = e.motoboy_id
INNER JOIN empresas e2 ON e2.id = m.empresa_id
WHERE e.deleted_at IS NULL AND e2.id = :companyId
AND DATE(e.created_at) = :dateParam
