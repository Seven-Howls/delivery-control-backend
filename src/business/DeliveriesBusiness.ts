...
export class DeliveriesBusiness {
    ...
    updateDeliveryStatusById = async (token: string, motoboyId: string, deliveryId:string, statusId: string)  => {
        try{
            //if(!token) throw new CustomError("Token ausente na autenticação",422);
            ...
            if(!delivery) throw new CustomError("Entrga não encontrada", 404);
